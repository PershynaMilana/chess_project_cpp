#include "game.h"
#include "button.h"
#include <QPixmap> //библиотека для рисования
#include "king.h"
#include <QDebug>
Game::Game(QWidget *parent ):QGraphicsView(parent)//класс "QGraphicsView"(для прорисовки 2д обьектов на поле)
                                                    //является базовым для дочернего класа game
{
    ...
    //настройки сцены
    setFixedSize(1400,900); //размеры
    setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff); //прокрутка
    setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    setScene(gameScene); //установка
    QBrush brush; //прорисовка
    brush.setStyle(Qt::SolidPattern);
    brush.setColor(Qt::black);
    setBackgroundBrush(brush);
    pieceToMove = NULL; //переменная фигуры для перемещения
...
void Game::drawChessBoard() //прорисовка поля
{
...
void Game::displayDeadWhite() //прорисовка поля для размещения убитых белых
{
    int SHIFT = 50;
    int j = 0;
    int k = 0;
...
void Game::displayDeadBlack() //прорисовка поля для размещения убитых чёрных
{
    int SHIFT = 50;
    int j = 0;
    int k = 0;
...
void Game::placeInDeadPlace(ChessPiece *piece)//убрать убитую фигуру в сторону
{
...
void Game::addToScene(QGraphicsItem *item) //добавление на поле
{
    gameScene->addItem(item);
}

void Game::removeFromScene(QGraphicsItem *item)//удаление с поля
{
    gameScene->removeItem(item);

}

QString Game::getTurn()//узнать очередь кто ходит
{
    return turn;
}

void Game::setTurn(QString value)//установить очередь
{
    turn = value;
}

void Game::changeTurn()//смена очереди
{
    if(getTurn() == "WHITE")
        setTurn("BLACK");
    else
...
void Game::drawDeadHolder(int x, int y,QColor color)//прорисовка поля для размещения убитых
{
...
void Game::displayMainMenu() //отоброжение начального меню
{
...
void Game::removeAll(){ //убрать всё после того как кто-то проиграл
